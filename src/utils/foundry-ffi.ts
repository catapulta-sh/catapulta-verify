...
import {
	getContractDataByArtifactAndBuildInfo,
	isBytecodeInArtifact,
	isBytecodeInBuildInfo,
} from "./bytecode.ts";
...
export const loadArtifacts = async (): Promise<any[]> => {
	...
	const filteredArtifactsFiles = importedArtifacts.filter(
		(y) => !!y?.bytecode && !(y?.bytecode?.object == "0x"),
	);
...
export const loadBuildInfo = async (
	...
	execSync(
		"cp .env .env.bk && sed -i.sedbak -r '/FOUNDRY_LIBRARIES/d' .env && rm .env.sedbak && sleep 3",
	);

	let forgeBuildCmd = "forge build --build-info --force";

	if (!!parsedRun?.libraries?.length) {
		forgeBuildCmd = `FOUNDRY_LIBRARIES="${parsedRun.libraries.join(
			",",
		)}" ${forgeBuildCmd}`;
	}
	...
	buildInfosInDir.forEach(async (file) => {
...
export const getSettingsByArtifact = async (
	...
	const foundArtifact = artifacts.find((artifact) =>
		isBytecodeInArtifact(bytecodeAndParams, artifact),
	);
	...
	if (!foundBuildInfo) {
		console.log(
			`Couldn't find build-info via bytecode for ${deploymentAddress}`,
		);
		console.log(bytecodeAndParams);
		return;
	}

	const contractData = getContractDataByArtifactAndBuildInfo(
		foundArtifact,
		foundBuildInfo,
	);
	...
	>contractData;
	const licenseType = extractOneLicenseFromSourceFile(
		contractInfo.sources[contractPath].content,
	);

	if (!licenseType) return;

	const rawParams =
		bytecodeAndParams.split(foundArtifact.bytecode.object)[1] || "";

	const metadata = foundArtifact.metadata;
	settings.apikey =
		etherscanApiKey ||
		ETHERSCAN_API_KEYS[chainId] ||
