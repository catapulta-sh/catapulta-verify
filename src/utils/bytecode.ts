export const isBytecodeInBuildInfo = (bytecode: string, buildInfo: any): boolean => {
  const contractRefs = Object.keys(buildInfo.output.contracts);
  for (let i = 0; i < contractRefs.length; i++) {
    const contractNames = Object.keys(buildInfo.output.contracts[contractRefs[i]]);
    for (let j = 0; j < contractNames.length; j++) {
      const contractName = contractNames[j];
      const contractInfo = buildInfo.output.contracts[contractRefs[i]][contractName];
      if (contractInfo.evm.bytecode.object.toLowerCase().startsWith(bytecode.substring(2).toLowerCase())) {
        return true;
      }
    }
  }
...
export const isBytecodeInArtifact = (bytecodeAndParams: string, artifact: any): boolean => {
  return !!bytecodeAndParams.toLowerCase().startsWith(artifact.bytecode.object.toLowerCase());
};
...
export const getContractDataByArtifactAndBuildInfo = (
  ...
  metadata.sources = Object.fromEntries(
    Object.keys(artifact.metadata.sources).map((key) => {
      ...
      if (!buildInfoSource) {
        const possibleKey = Object.keys(buildInfo.input.sources).find((x) => x.endsWith(key));
        if (possibleKey) {
  ...
  return {
    contractName: `${Object.keys(artifact.metadata.settings.compilationTarget)[0]}:${
